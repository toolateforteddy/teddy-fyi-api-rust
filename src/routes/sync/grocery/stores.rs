...
struct Pending {
    ...
    up_name: Vec<String>,
    up_position: Vec<i32>,
    up_is_default_supported: Vec<bool>,
    ...
    async fn flush(
        ...
        if !self.up_id.is_empty() {
            sqlx::query!(
                ...
                SELECT v.id, v.name, v.position, v.is_default_supported, $8, v.list_id,
                       v.version, v.is_deleted, 'SYNCED', $9, $10
                FROM UNNEST($1::text[], $2::text[], $3::int4[], $4::bool[], $5::text[], $6::int4[], $7::bool[])
                    AS v(id, name, position, is_default_supported, list_id, version, is_deleted)
            ...
            self.up_is_default_supported.clear();
        ...
        if !self.del_id.is_empty() {
            ...
            for (id, status_idx) in self.del_id.iter().zip(self.del_status_idx.iter()) {
                let version = match deleted.get(id) {
                    ...
                    None => ack_unsynced_delete("store", id),
                };
            ...
            OperationType::Insert | OperationType::Update => {
                ...
                if let Some(ref data) = change.data {
                        ...
                        Ok(item) => {
                            ...
                            let next_version = if let Some(row) = record {
                                if matches!(change.operation_type, OperationType::Update) && change.version < row.version {
                                    tracing::warn!(
                                        "Conflicting write for store {} (client version {}, server version {}); accepting it as the later arrival",
                                        change.id, change.version, row.version
                                    );
                                }
                                advance_version("Store", &change.id, row.version)?
                            } else {
                                seed_version("Store", &change.id, item.version)?
                            };
                            ...
                            pending.up_is_default_supported.push(item.is_default_supported);
                    ...
                    if let Some(row) = record {
                        ...
                        let next_version = advance_version("Store", &change.id, row.version)?;
